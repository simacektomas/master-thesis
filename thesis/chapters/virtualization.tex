...
Na druhou stranu procesy využívají ke svému běhu mimo uživatelské části \textbf{ISA} také \textbf{ABI} nebo v případě vyšších
programovacích jazyků \textbf{API}. Virtuální stroje, které se specializují na virtualizaci těchto dvou systémových 
...
systémy, neboť každý OS má k dispozici vlastní disjunktní prostředí, ve kterém může operovat. Pokud nastane SW nebo HW chyby v
jedné části (partition) systému, aplikace a OS běžící v ostatních částech nejsou nijak omezeny.
...
Výstup můžeme vidět na obrázku \ref{figure:consolidation_img} vpravo.
\begin{figure}
    \centering    
...
těmito zastaralými systémy, můžeme je spustit jako virtuální počítač na modernějším hradware. Systém se bude chovat stejně jako
kdyby běžel na zastaralém hradware, zatímco výkonost služby může těžit z novější a výkonnější hardwarové vrstvy \cite{oracle:virtualization:reasons}.
\subsection{Izolace}
